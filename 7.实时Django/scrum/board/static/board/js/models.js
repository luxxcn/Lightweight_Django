(function ($, Bankbone, _, app) {
    ...
    $.ajaxPrefilter(function(settings, originalOptions, xhr) {
        ...
        if (!csrfSafeMethod(settings.type) && !this.crossDomain) {
            // Send the token to same origin, ralative URLs only.
    ...
    var Session = Bankbone.Model.extend({
        defaults: {
            token: null
        },
    ...
    var BaseCollection = Backbone.Collection.extend({
        parse: function (response) {
            ...
            return response || []; // Sprints 获取到的response直接是结果集合，没有results键
        },
